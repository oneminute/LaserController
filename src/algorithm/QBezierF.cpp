...
/*!
  \internal
*/
QBezierF QBezierF::fromPoints(const QPointF &p1, const QPointF &p2,
...
/*!
  \internal
*/
QPolygonF QBezierF::toPolygon(qreal bezier_flattening_threshold) const
{
...
QBezierF QBezierF::mapBy(const QTransform &transform) const
{
    return QBezierF::fromPoints(transform.map(pt1()), transform.map(pt2()), transform.map(pt3()), transform.map(pt4()));
}

QBezierF QBezierF::getSubRange(qreal t0, qreal t1) const
{
    QBezierF result;
    QBezierF temp;
...
void QBezierF::addToPolygon(QPolygonF *polygon, qreal bezier_flattening_threshold) const
{
    QBezierF beziers[10];
    int levels[10];
    beziers[0] = *this;
...
//void QBezierF::addToPolygon(QDataBuffer<QPointF> &polygon, qreal bezier_flattening_threshold) const
//{
//    QBezierF beziers[10];
//    int levels[10];
//    beziers[0] = *this;
...
static ShiftResult good_offset(const QBezierF *b1, const QBezierF *b2, qreal offset, qreal threshold)
{
...
static ShiftResult shift(const QBezierF *orig, QBezierF *shifted, qreal offset, qreal threshold)
{
    int map[4];
    ...
    *shifted = QBezierF::fromPoints(points_shifted[map[0]], points_shifted[map[1]],
...
static bool addCircle(const QBezierF *b, qreal offset, QBezierF *o)
{
    QPointF normals[3];
...
int QBezierF::shifted(QBezierF *curveSegments, int maxSegments, qreal offset, float threshold) const
{
...
qreal QBezierF::length(qreal error) const
{
...
void QBezierF::addIfClose(qreal *length, qreal error) const
{
    QBezierF left, right;     /* bez poly splits */
...
qreal QBezierF::tForY(qreal t0, qreal t1, qreal y) const
{
    ...
    do {
        ...
        qreal a, b, c, d;
        QBezierF::coefficients(t, a, b, c, d);
...
int QBezierF::stationaryYPoints(qreal &t0, qreal &t1) const
{
...
qreal QBezierF::tAtLength(qreal l) const
{
    qreal len = length();
    qreal t   = qreal(1.0);
...
QBezierF QBezierF::bezierOnInterval(qreal t0, qreal t1) const
{
    if (t0 == 0 && t1 == 1)
        return *this;

    QBezierF bezier = *this;

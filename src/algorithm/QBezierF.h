...
{
public:
    static QBezierF fromPoints(const QPointF &p1, const QPointF &p2,
    ...
    QBezierF mapBy(const QTransform &transform) const;
    ...
    inline void parameterSplitLeft(qreal t, QBezierF *left);
    inline void split(QBezierF *firstHalf, QBezierF *secondHalf) const;

    int shifted(QBezierF *curveSegments, int maxSegmets,
                qreal offset, float threshold) const;

    QBezierF bezierOnInterval(qreal t0, qreal t1) const;
    QBezierF getSubRange(qreal t0, qreal t1) const;

    qreal x1, y1, x2, y2, x3, y3, x4, y4;
};

inline QPointF QBezierF::midPoint() const
{
    return QPointF((x1 + x4 + 3*(x2 + x3))/8., (y1 + y4 + 3*(y2 + y3))/8.);
}

inline QLineF QBezierF::midTangent() const
{
...
inline QLineF QBezierF::startTangent() const
{
...
inline QLineF QBezierF::endTangent() const
{
...
inline void QBezierF::coefficients(qreal t, qreal &a, qreal &b, qreal &c, qreal &d)
{
    qreal m_t = 1. - t;
    b = m_t * m_t;
    c = t * t;
...
inline QPointF QBezierF::pointAt(qreal t) const
{
...
inline QPointF QBezierF::normalVector(qreal t) const
{
    qreal m_t = 1. - t;
    qreal a = m_t * m_t;
    qreal b = t * m_t;
...
inline QPointF QBezierF::derivedAt(qreal t) const
{
...
inline QPointF QBezierF::secondDerivedAt(qreal t) const
{
    qreal a = 2. - 2. * t;
    qreal b = -4 + 6 * t;
    qreal c = 2 - 6 * t;
...
inline void QBezierF::split(QBezierF *firstHalf, QBezierF *secondHalf) const
{
    Q_ASSERT(firstHalf);
    Q_ASSERT(secondHalf);
...
inline void QBezierF::parameterSplitLeft(qreal t, QBezierF *left)
{
    left->x1 = x1;
    left->y1 = y1;

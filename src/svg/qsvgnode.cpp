...
void QSvgNode::appendStyleProperty(QSvgStyleProperty *prop, const QString &id)
{
    ...
    case QSvgStyleProperty::FILL:
        ...
        if (m_style.fill->style())
        {
            m_style.fill->style()->adjustByNode(this);
        }
        break;
...
bool QSvgNode::hasStyle() const
{
    const QSvgNode *node = this;
    if (!node)
        return false;
    return node->m_style.quality || node->m_style.fill || node->m_style.font || node->m_style.stroke || node->m_style.solidColor
            || node->m_style.gradient || node->m_style.opacity;
}

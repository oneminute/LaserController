...
{
    QPainterPath path;
    QRectF realBounds = m_bounds;
    realBounds.moveTo(0, 0);
    realBounds.setRight(m_bounds.right());
    realBounds.setBottom(m_bounds.bottom());
    path.addEllipse(m_bounds);
//    path.translate(-m_bounds.x(), -m_bounds.y());
...
void QSvgEllipse::draw(QPainter *p, QSvgExtraStates &states)
{
    ...
    QRectF realBounds = m_bounds;
    realBounds.moveTo(0, 0);
    QT_SVG_DRAW_SHAPE(p->drawEllipse(realBounds));
...
void QSvgImage::draw(QPainter *p, QSvgExtraStates &states)
{
    ...
    QRectF realBounds = m_bounds;
    realBounds.moveTo(0, 0);
    p->drawImage(realBounds, m_image);
    revertStyle(p, states);
}
...
{
    QRectF realBounds = m_rect;
    realBounds.moveTo(0, 0);
    applyStyle(p, states);
    if (m_rx || m_ry) {
        QT_SVG_DRAW_SHAPE(p->drawRoundedRect(realBounds, m_rx, m_ry, Qt::RelativeSize));
    } else {
        QT_SVG_DRAW_SHAPE(p->drawRect(realBounds));
    }
    revertStyle(p, states);
}

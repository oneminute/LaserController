...
void QSvgStrokeStyle::apply(QPainter *p, const QSvgNode *, QSvgExtraStates &states)
{
    ...
    if (m_strokeDashArraySet) {
        if (m_stroke.style() == Qt::SolidLine) {
            ...
            // If both m_width and dash array was set, the dash array is already scaled correctly.
            pen.setDashPattern(m_stroke.dashPattern());
            setDashOffsetNeeded = true;
        } else {
            // If dash array was set, but not the m_width, the dash array has to be scaled with respect to the old m_width.
        ...
        // If the m_width was set, but not the dash array, the old dash array must be scaled with respect to the new m_width.
...
void QSvgGradientStyle::adjustByNode(QSvgNode *node)
{
    if (!node)
        return;
    ...
    {
        QLinearGradient* lg = reinterpret_cast<QLinearGradient*>(m_gradient);
        if (lg)
        {
            qreal xDiff = lg->finalStop().x() - lg->start().x();
            qreal yDiff = lg->finalStop().y() - lg->start().y();
            lg->setStart(0, 0);
            lg->setFinalStop(xDiff, yDiff);
        }
    }
        break;
    case QGradient::RadialGradient:
    {
        QRadialGradient* rg = reinterpret_cast<QRadialGradient*>(m_gradient);
        if (rg)
        {
            QSizeF bounds = node->transformedBounds().size();
//            qreal r = rg->radius();
            rg->setCenter(bounds.width() / 2, bounds.height() / 2);
            rg->setFocalPoint(bounds.width() / 2, bounds.height() / 2);
//            rg->setCenterRadius(r);
        }
    }
        break;

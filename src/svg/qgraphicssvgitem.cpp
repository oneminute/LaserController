...
class QGraphicsSvgItemPrivate : public QSharedData
{
public:
    ...
    {
        q_ptr->setParentItem(parent);
        renderer = new QSvgRenderer(q_ptr);
        QObject::connect(renderer, SIGNAL(repaintNeeded()),
                         q_ptr, SLOT(_q_repaintItem()));
        q_ptr->setCacheMode(QGraphicsItem::DeviceCoordinateCache);
        q_ptr->setMaximumCacheSize(QSize(1024, 768));
    }
    ...
    inline void updateDefaultSize()
    {
        ...
        q_ptr->setX(bounds.x() * factor);
        q_ptr->setY(bounds.y() * factor);
//        qDebug() << "x=" << q_ptr->x() << ", y=" << q_ptr->y() << ", factor=" << factor;
        q_ptr->setScale(factor);
    }

    QGraphicsSvgItem* q_ptr;
...
QGraphicsSvgItem::QGraphicsSvgItem(QGraphicsItem *parent)
    : QGraphicsObject(0)
    , d_ptr(new QGraphicsSvgItemPrivate)
{
    d_ptr->q_ptr = this;
    d_ptr->init(parent);
}
...
//    Q_UNUSED(option);
    ...
    if (!d_ptr->renderer->isValid())
        return;

//    if (d_ptr->elemId.isEmpty())
//        d_ptr->renderer->render(painter, d_ptr->boundingRect);
//    else
//        d_ptr->renderer->render(painter, d_ptr->elemId, d_ptr->boundingRect);
    QSvgExtraStates states;
    d_ptr->renderer->render(painter, states);
...
{
    QGraphicsItem::setData(Extra::ExtraMaxDeviceCoordCacheSize, size);
    update();
}
...
{
    return QGraphicsItem::data(Extra::ExtraMaxDeviceCoordCacheSize).toSize();
}
...
void QGraphicsSvgItem::setFactor(qreal factor)
{
    d_ptr->factor = factor;
}

qreal QGraphicsSvgItem::factor() const
{
    return d_ptr->factor;
}
...
/*!
    ...
    this item is used.
*/
void QGraphicsSvgItem::setRenderer(QSvgRenderer *renderer)
{
    if (d_ptr->renderer)
        delete d_ptr->renderer;

    d_ptr->renderer = renderer;
//    d_ptr->shared = true;
